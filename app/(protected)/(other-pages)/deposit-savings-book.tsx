...
export default function DepositSavingsBookScreen() {
  ...
  const handleSave = async () => {
    ...
    if (isNaN(amt) || amt <= 0) {
      showAlert({ title: 'Lỗi', message: 'Nhập số tiền hợp lệ', icon: 'error' });
      return;
    }
    try {
      setSaving(true);
      await depositSavingsBook(id, {
        amount: amt,
      ...
      showAlert({ title: 'Lỗi', message: err instanceof Error ? err.message : 'Không thể gửi thêm', icon: 'error' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const selected = sourceId ? flat.find(m => m.id === sourceId) : null;
    ...
    return (
      <SafeAreaView style={s.container}>
        <View style={[s.header, { backgroundColor: isDark ? themeColors.cardGlass : themeColors.tint }]}>
          ...
          <Text style={[s.headerTitle, { color: headerFgColor }]}>Gửi thêm</Text>
          <View style={{ width: 40 }} />
        </View>
    ...
    <SafeAreaView style={s.container}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content}>
        <View style={[s.card, { backgroundColor: themeColors.card }]}>
          <Text style={[s.label, { color: themeColors.textSecondary }]}>Số tiền</Text>
          <TextInput
        ...
        <TouchableOpacity style={[s.row, { backgroundColor: themeColors.card }]} onPress={() => setShowModal(true)}>
          <Text style={{ color: themeColors.text }}>{selected ? selected.name : 'Chọn tài khoản nguồn'}</Text>
      ...
      <Modal visible={showModal} transparent animationType="slide">
        <TouchableOpacity style={s.modalOverlay} activeOpacity={1} onPress={() => setShowModal(false)}>
          <View style={[s.modalContent, { backgroundColor: themeColors.card }]}>
            {flat.map(ms => (
              <TouchableOpacity key={ms.id} style={s.modalItem} onPress={() => { setSourceId(ms.id); setShowModal(false); }}>
